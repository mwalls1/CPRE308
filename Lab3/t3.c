...
void *producer(void *arg)
{
  ...
  while (!producer_done)
  {
	...
	pthread_mutex_lock(&mut);//lock mutex
    	supply = supply + NUM_ITEMS_PER_PRODUCE;//create an initial supply
    	pthread_cond_broadcast(&consumer_cv);//tell consumers we are ready
        pthread_mutex_unlock(&mut);//unlock mutex

	while (!producer_done)//create second loop to keep production at a minimum
	{
		pthread_mutex_lock(&mut);//lock mutex
		while(supply == 0)//wait for supply to be 0
			pthread_cond_wait(&producer_cv, &mut);//wait for a consumer to tell us to make more
    		supply = supply + NUM_ITEMS_PER_PRODUCE;//increase supply by 10
    		pthread_cond_broadcast(&consumer_cv);//tell consumers we are ready
        	pthread_mutex_unlock(&mut);//unlock mutex
		if(num_cons_remaining == 0)//if there are no consumers left, close out of both loops
			producer_done = 1;
	}

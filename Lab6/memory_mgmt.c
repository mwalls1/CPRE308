...
int PRAlgo_FIFO(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	...
	int i = 1;
	int min = PageFrames[0].time_of_arrival;
	for(i; i < num_frames; i++)
	{
		if(PageFrames[i].time_of_arrival < min)
		{
			min = PageFrames[i].time_of_arrival;
			frame_to_evict = i;
		}
	}
...
int PRAlgo_LRU(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	...
	/*TODO: fill in the code below */
	int i = 1;
	int min = PageFrames[0].time_of_access;
	for(i; i < num_frames; i++)
	{
		if(PageFrames[i].time_of_access < min)
		{
			min = PageFrames[i].time_of_access;
			frame_to_evict = i;
		}
	}
...
int PRAlgo_OPT(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	...
	int id = PageFrames[0].page_id;
	int max = 0;
	int pageFound = 0;
	for(i; i < num_accesses; i++)
	{
		if(PageAccesses[i] == id)
		{
			max = i;
			pageFound = 1;
			break;
